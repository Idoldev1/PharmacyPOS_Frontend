...
import { inventoryService, type CreateDrugPayload } from "../../services/inventoryService";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <p className="mb-4 text-sm text-slate-500">{drug.name}</p>
        <div className="mb-4">
...
export default function InventoryPage() {
  ...
  const totalValue = drugs.reduce((sum, d) => sum + d.stockQty * d.unitCost, 0);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {["Drug Name", "Category", "Strength/Form", "Batch No", "Expiry Date", "Stock", "Reorder", "Unit Cost", "Selling Price", "Status"].map((h) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                <tr>
                  <td colSpan={10} className="px-4 py-8 text-center text-slate-400">Loading inventory…</td>
                </tr>
              ) : drugs.length === 0 ? (
                <tr>
